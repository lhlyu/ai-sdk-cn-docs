...
该包提供了一个可序列化的状态机和一个切片运行器，您可以从自己的 `'use workflow'` 和 `'use step'` 函数中调用它。

核心的 harness 和工作流文件是框架无关的。下面展示的 HTTP 处理程序和 Next.js 配置是如何从一个应用框架中暴露工作流的示例；请根据您的运行时和 Workflow SDK 集成调整这些部分。
...
有了智能体定义文件后，从两个工作流特定的部分开始：一个切片步骤，用于运行 harness 轮次中一个有时间限制的部分；以及一个工作流，用于调用该步骤直到轮次完成。
...
从服务器端代码启动工作流。您可以从 HTTP 端点返回 `run.readable`，直接消费它，或者通过您框架的流式响应原语暴露它。
...
这是核心的工作流/harness 集成。当您需要一个多轮聊天在多次工作流运行中重新连接到同一个原生 harness 会话时，请添加恢复持久化。

### HTTP 路由示例

此示例使用 Next.js 的 `Request`/`Response` 处理程序和 AI SDK UI 消息流。对于其他框架，工作流相关代码类似：提取最新的用户消息，调用 `start(codingWorkflow, [...])`，并返回运行的 readable 流。
...
如果您使用 Next.js，请使用 `withWorkflow()` 包装您的配置，以便 Workflow SDK 转换 `'use workflow'` 和 `'use step'` 模块。其他框架有自己的 Workflow SDK 设置。
...
仅持久化在完成的轮次后返回的不透明 `resumeFrom` 状态。下面的示例使用 Workflow 步骤，因为文件系统访问必须保持在工作流函数之外。在生产环境中，请使用持久化存储而不是本地文件。
...
harness 的 `sessionId` 为沙箱提供了跨工作流运行的稳定身份，并允许工作流在发送下一条用户消息之前加载上一轮次的恢复状态。

一个 harness 会话拥有其原生的对话历史记录，因此路由只发送最新的用户消息。不要将完整的 UI 消息历史记录重放给 harness。

## 工作原理

每个用户轮次由一个 `HarnessWorkflowState` 表示：

- `createHarnessWorkflowState()` 为轮次创建初始状态。
- `runHarnessAgentSlice()` 流式传输轮次的一个有时间限制的切片。
...
- 您的路由或服务器入口点启动工作流。它可以导入 `ai` 辅助工具，例如 `convertToModelMessages()` 和 `createUIMessageStreamResponse()`。

不要在与路由处理程序、服务器入口点或智能体相同的模块中定义工作流函数。Workflow DevKit 会编译从 `'use workflow'` 指令可达的模块图。保持图的小型化可以防止将 Node 重型的智能体、沙箱和框架依赖项拉入工作流包中。
