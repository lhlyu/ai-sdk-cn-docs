---
title: 流协议
description: 了解 AI SDK 中支持的流协议。
---

# 流协议

AI SDK 的 UI 函数（如 `useChat` 和 `useCompletion`）同时支持文本流和数据流。
流协议定义了数据如何在 HTTP 协议之上流式传输到前端。

本页面介绍了这两种协议以及如何在后端和前端使用它们。

你可以利用这些信息为你的用例开发自定义后端和前端，例如，
提供用其他语言（如 Python）实现的兼容 API 端点。

例如，以下是一个使用 [FastAPI](https://github.com/vercel/ai/tree/main/examples/next-fastapi) 作为后端的示例。
...
当你使用 `useChat` 或 `useCompletion` 时，需要通过将 `streamProtocol` 选项设置为 `text` 来启用文本流。
...
数据流协议使用服务器发送事件（SSE）格式，以实现更好的标准化、通过 ping 保持连接、重连能力以及更好的缓存处理。

<Note>
  当你从自定义后端提供数据流时，需要将 `x-vercel-ai-ui-message-stream` 头部设置为 `v1`。
</Note>
...
推理文件部分包含对推理过程中生成的文件（如推理过程中产生的图像）的引用。
...
自定义部分表示提供商特定的内容，这些内容不属于标准部分类型。`kind` 字段以 `{provider}.{provider-type}` 格式标识特定的自定义内容类型。
...
自定义数据部分允许流式传输任意结构化数据，并具有类型特定的处理。
...
当省略 `isAutomatic` 时，请求期望客户端提供明确的审批响应。
...
对于由提供商执行的工具，响应还可以包含 `providerExecuted: true`。
...
此部分对于正确处理多个拼接的助手调用是必要的，例如在后端调用工具时，同时使用 `useChat` 中的步骤。
