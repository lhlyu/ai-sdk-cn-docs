...
数据流协议使用服务器发送事件（SSE）格式，以提高标准化程度、通过 ping 保持连接、支持重连以及更好的缓存处理。

<Note>
  当你从自定义后端提供数据流时，你需要将 `x-vercel-ai-ui-message-stream` 头设置为 `v1`。
</Note>
...
文本内容使用开始/增量/结束模式流式传输，每个文本块具有唯一 ID。
...
推理内容使用开始/增量/结束模式流式传输，每个推理块具有唯一 ID。
...
推理文件部分包含对推理过程中生成文件的引用，例如推理过程中生成的图像。
...
当省略 `isAutomatic` 时，请求期望客户端给出明确的审批响应。`reason` 是可选的，解释为什么工具调用需要审批。
...
移除自最近的 `start-step` 部分以来收到的所有消息部分。如果
没有步骤边界，则移除当前消息中的所有部分。当流式步骤被重试，并且必须使失败尝试的部分输出失效后再发送替换输出时，这很有用。
...
数据流协议在前端由 `useChat` 和 `useCompletion` 支持，并且默认使用。
