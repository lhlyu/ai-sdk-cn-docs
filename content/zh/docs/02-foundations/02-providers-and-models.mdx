...
像 OpenAI 和 Anthropic 这样的公司（即提供商）通过其自有 API 提供对一系列具有不同优势和能力的大语言模型（LLM）的访问。

每个提供商通常都有自己独特的方式与其模型交互，这使得切换提供商的过程变得复杂，并增加了供应商锁定的风险。

为了解决这些挑战，AI SDK Core 提供了一种标准化的方法来与 LLM 交互，该方法通过一个[语言模型规范](https://github.com/vercel/ai/tree/main/packages/provider/src/language-model/v3)来抽象不同提供商之间的差异。这种统一接口允许您在轻松切换提供商的同时，对所有提供商使用相同的 API。
