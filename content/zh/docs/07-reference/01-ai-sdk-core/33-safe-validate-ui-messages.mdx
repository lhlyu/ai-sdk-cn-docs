...
`safeValidateUIMessages` 是一个异步函数，用于验证 UI 消息，其功能与 [`validateUIMessages`](/docs/reference/ai-sdk-core/validate-ui-messages) 类似，但不会抛出异常，而是返回一个包含 `success` 键以及 `data` 或 `error` 的对象。
