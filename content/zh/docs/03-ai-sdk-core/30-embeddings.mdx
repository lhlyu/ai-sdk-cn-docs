...
嵌入是一种将单词、短语或图像表示为高维空间向量的方法。
在这个空间中，相似的单词彼此靠近，单词之间的距离可用于衡量它们的相似度。

## 嵌入单个值

AI SDK 提供了 [`embed`](/docs/reference/ai-sdk-core/embed) 函数来嵌入单个值，这对于查找相似单词或短语、或对文本进行聚类等任务非常有用。
...
在加载数据时，例如为检索增强生成（RAG）准备数据存储时，通常需要一次嵌入多个值（批量嵌入）。
...
可以使用 `providerOptions` 为特定于提供商的参数配置嵌入模型设置：
...
`embed` 和 `embedMany` 都接受一个可选的 `abortSignal` 参数，
...
你可以使用 `wrapEmbeddingModel` 和 `EmbeddingModelMiddleware` 来增强嵌入模型，例如设置默认值。
