...
`transcribe` 接受一个可选的 `abortSignal` 参数，类型为
...
`transcribe` 接受一个可选的 `headers` 参数，类型为 `Record<string, string>`，
你可以用它来向转录请求添加自定义请求头。
...
当 `transcribe` 无法生成有效的转录文本时，会抛出 [`AI_NoTranscriptGeneratedError`](/docs/reference/ai-sdk-errors/ai-no-transcript-generated-error)。
...
- `responses`：关于语音转文字模型响应的元数据，包括时间戳、模型和请求头。
