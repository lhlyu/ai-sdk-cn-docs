...
当您有实时原始音频并需要在完整音频流完成之前获取转录更新时，请使用 `experimental_streamTranscribe`。该函数使用支持流式传输的转录模型；提供商选项用于配置特定提供商的行为，但流式操作由函数本身选择。
...
当您同时需要流片段和最终结果时，请先访问 `fullStream`，并在消费它期间或之后等待结果 Promise。先访问结果 Promise 会在内部消费流，因此 `fullStream` 将不再可用。这样可以避免为实时音频保留无界的重放缓冲区。
...
<Note>
  字符串模型 ID 通过全局提供商（默认为 AI Gateway）解析。
  AI Gateway 支持受支持模型的流式转录（例如 `openai/gpt-realtime-whisper`、`elevenlabs/eleven-scribe-2-realtime`、
  `xai/grok-stt`），因此字符串 ID 可以正常工作：
  `experimental_streamTranscribe({ model: 'openai/gpt-realtime-whisper', ...
  })`。您也可以传递提供商模型实例（例如
  `openai.transcription('gpt-realtime-whisper')`）以直接向提供商进行流式传输。
</Note>
...
对请求/响应和流式转录使用相同的 `xai.transcription()` 模型；
`experimental_streamTranscribe` 选择提供商的 WebSocket STT 传输。
...
当 `audio` 是 URL 时，SDK 会以默认的 **2 GiB** 大小限制下载文件。
...
以上是 AI SDK 提供商支持的转录模型的一小部分。更多信息，请参阅相应的提供商文档。
