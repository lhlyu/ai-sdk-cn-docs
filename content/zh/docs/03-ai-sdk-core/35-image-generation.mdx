...
<Note>
  `generateImage` 将根据需要自动（并行）调用模型，以生成所请求数量的图像。
</Note>

每个图像模型在单次 API 调用中都有一个内部限制，限制其可以生成的图像数量。AI SDK 通过在您使用 `n` 参数请求多张图像时适当批处理请求来自动管理此限制。默认情况下，SDK 使用提供商文档中记录的限制（例如，DALL-E 3 每次调用只能生成 1 张图像，而 DALL-E 2 最多支持 10 张）。

如有需要，您可以在生成图像时使用 `maxImagesPerCall` 设置覆盖此行为。这在处理新模型或自定义模型时特别有用，因为默认的批处理大小可能不是最优的：
...
const { images } = await generateImage({
  ...
  n: 10, // 将进行 2 次调用，每次 5 张图像
});
```

### 提供种子

您可以向 `generateImage` 函数提供种子来控制图像生成过程的输出。
如果模型支持，相同的种子将始终生成相同的图像。
...
您可以使用 `providerOptions` 参数将这些设置传递给 `generateImage` 函数。提供商的选项（如下例中的 `openai`）将成为请求体属性。
...
`generateImage` 接受一个可选的 `abortSignal` 参数，类型为 [`AbortSignal`](https://developer.mozilla.org/en-US/docs/Web/API/AbortSignal)，您可以使用它来中止图像生成过程或设置超时。
...
`generateImage` 接受一个可选的 `headers` 参数，类型为 `Record<string, string>`，您可以使用它为图像生成请求添加自定义请求头。
...
某些提供商会为单个图像公开元数据。这些元数据可直接在生成的图像上获取：
...
当请求被拆分为多个提供商调用时，`calls` 会保留每次调用的图像、提供商元数据、响应元数据、警告和使用情况。`images` 和 `usage` 分别保持便捷的扁平化视图和聚合视图。
...
当 `generateImage` 无法生成有效图像时，它会抛出 [`AI_NoImageGeneratedError`](/docs/reference/ai-sdk-errors/ai-no-image-generated-error)。

此错误在 AI 提供商无法生成图像时发生。可能由以下原因引起：
...
以上是 AI SDK 提供商支持的图像模型的一小部分。更多信息，请参阅相应的提供商文档。
