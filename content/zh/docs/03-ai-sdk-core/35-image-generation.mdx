...
<Note>
  `generateImage` 会根据需要自动（并行）调用模型，以生成请求数量的图像。
</Note>

每个图像模型在单次 API 调用中都有内部限制，规定其能生成的最大图像数量。AI SDK 会自动管理此过程：当你使用 `n` 参数请求多张图像时，SDK 会适当地对请求进行批处理。默认情况下，SDK 使用提供商文档中规定的限制（例如，DALL-E 3 每次调用只能生成 1 张图像，而 DALL-E 2 最多支持 10 张）。

如果需要，你可以在生成图像时使用 `maxImagesPerCall` 设置覆盖此行为。这在处理默认批处理大小可能不是最优的新模型或自定义模型时特别有用：
...
你可以向 `generateImage` 函数提供种子值，以控制图像生成过程的输出。
...
你可以使用 `providerOptions` 参数将这些设置传递给 `generateImage` 函数。提供商（如下例中的 `openai`）的选项将成为请求体属性。
...
`generateImage` 接受一个可选的 `abortSignal` 参数，类型为 [`AbortSignal`](https://developer.mozilla.org/en-US/docs/Web/API/AbortSignal)，你可以使用它来中止图像生成过程或设置超时。
...
`generateImage` 接受一个可选的 `headers` 参数，类型为 `Record<string, string>`，你可以使用它向图像生成请求添加自定义请求头。
...
如果模型返回警告（例如，关于不支持的参数），这些警告将在响应的 `warnings` 属性中可用。
...
某些语言模型（例如 Google 的 `gemini-2.5-flash-image`）支持包括图像在内的多模态输出。
使用此类模型时，你可以通过响应的 `files` 属性访问生成的图像。
...
| [Amazon Bedrock](/providers/ai-sdk-providers/amazon-bedrock#image-models)       | `amazon.nova-canvas-v1:0`                                    | 320-4096（16 的倍数），1:4 到 4:1，最大 420 万像素                                                                                                             |
...
以上只是 AI SDK 提供商支持的图像模型的一小部分。更多信息，请参阅相应提供商的文档。
