...
AI SDK 提供了 [`generateSpeech`](/docs/reference/ai-sdk-core/generate-speech) 函数，用于使用语音模型从文本生成语音。
...
```ts
const audioData = result.audio.uint8Array; // 音频数据，格式为 Uint8Array
// 或
const audioBase64 = result.audio.base64; // 音频数据，格式为 base64 字符串
```
...
你可以使用 `providerOptions` 参数设置模型特定的选项。
...
`generateSpeech` 接受一个可选的 `abortSignal` 参数，类型为 [`AbortSignal`](https://developer.mozilla.org/en-US/docs/Web/API/AbortSignal)，你可以用它来中止语音生成过程或设置超时。
...
`generateSpeech` 接受一个可选的 `headers` 参数，类型为 `Record<string, string>`，你可以用它向语音生成请求添加自定义请求头。
...
警告信息（例如不支持的参数）可通过 `warnings` 属性获取。
...
当 `generateSpeech` 无法生成有效的音频时，它会抛出一个 [`AI_NoSpeechGeneratedError`](/docs/reference/ai-sdk-errors/ai-no-speech-generated-error) 错误。
...
以上只是 AI SDK 提供商支持的语音模型中的一小部分。更多信息，请参阅相应提供商的文档。
