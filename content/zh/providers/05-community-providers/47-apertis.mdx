---
title: Apertis
description: Apertis AI SDK 模型提供商
---

# Apertis 提供商

[Apertis](https://apertis.ai) 是一个统一的 AI 网关，通过单一 API 提供来自 OpenAI、Anthropic、Google 等领先提供商的 470 多个模型。

## 主要特性

- **单一 API 密钥**：使用一个 API 密钥即可访问来自多个提供商的 470+ 个模型
- **按需付费**：透明定价，无月费
...
您可以导入默认的 `apertis` 提供商实例，或创建一个自定义实例：
...
从 [Apertis 控制台](https://apertis.ai/token) 获取您的 API 密钥。
...
- **其他**：`glm-4.7`、`minimax-m2.1` 以及 470+ 更多模型
