...
[Aihubmix](https://aihubmix.com/) 提供商通过 Aihubmix API 提供对多个 AI 提供商的统一访问，包括 OpenAI、Anthropic Claude 和 Google Gemini 模型。在 [aihubmix.com/models](https://aihubmix.com/models) 查看所有可用模型。

## 安装

Aihubmix 提供商位于 `@aihubmix/ai-sdk-provider` 模块中。你可以通过以下方式安装：
...
你可以从 [Aihubmix Keys](https://aihubmix.com/token) 获取你的 Aihubmix API 密钥。

### 方法二：使用环境变量

或者，你可以通过设置 `AIHUBMIX_API_KEY` 环境变量来使用预配置的 `aihubmix` 实例：
...
console.log('Token 使用量:', await result.usage);
