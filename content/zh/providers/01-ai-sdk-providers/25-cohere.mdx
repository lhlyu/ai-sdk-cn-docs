...
Cohere 提供商位于 `@ai-sdk/cohere` 模块中。你可以通过以下方式安装：
...
你可以从 `@ai-sdk/cohere` 导入默认的提供商实例 `cohere`：

```ts
import { cohere } from '@ai-sdk/cohere';
```

如果你需要自定义设置，可以从 `@ai-sdk/cohere` 导入 `createCohere`，并使用你的设置创建提供商实例：
...
你可以使用以下可选设置来自定义 Cohere 提供商实例：

- **baseURL** _string_

  为 API 调用使用不同的 URL 前缀，例如使用代理服务器。
  默认前缀是 `https://api.cohere.com/v2`。
...
你可以使用提供商实例创建调用 [Cohere 聊天 API](https://docs.cohere.com/v2/docs/chat-api) 的模型。
...
你可以使用 Cohere 语言模型通过 `generateText` 函数生成文本：
...
Cohere 语言模型也可以用于 `streamText` 函数，
...
<Note>
  上表列出了常用模型。请参阅 [Cohere 文档](https://docs.cohere.com/v2/docs/models#command) 获取完整可用模型列表。如有需要，你也可以将任何可用的提供商模型 ID 作为字符串传递。
</Note>

#### 图像输入

支持视觉能力的 Cohere 模型（如 `command-a-vision-07-2025`）接受图像输入作为消息内容的一部分。你可以将图像作为原始字节、base64 编码字符串或 URL 传递：
...
你可以在文件部分使用 `cohere` 提供商选项来设置[图像输入细节](https://docs.cohere.com/docs/image-inputs)为 `high`、`low` 或 `auto`：
...
Cohere 通过 `command-a-reasoning-08-2025` 模型引入了推理功能。你可以在 https://docs.cohere.com/docs/reasoning 了解更多信息。
...
你可以使用 `.embedding()` 工厂方法创建调用 [Cohere 嵌入 API](https://docs.cohere.com/v2/reference/embed) 的模型。

```ts
const model = cohere.embedding('embed-english-v3.0');
```

你可以使用 Cohere 嵌入模型通过 `embed` 函数生成嵌入：
...
- **inputType** _'search_document' | 'search_query' | 'classification' | 'clustering'_
  ...
  - `search_query`：用于针对向量数据库运行搜索查询以查找相关文档的嵌入。
...
- **truncate** _'NONE' | 'START' | 'END'_
  ...
  - `START`：将丢弃输入的开头，直到剩余输入恰好等于模型的最大输入令牌长度。
  - `END`：将丢弃输入的结尾，直到剩余输入恰好等于模型的最大输入令牌长度。
...
你可以使用 `.reranking()` 工厂方法创建调用 [Cohere 重排序 API](https://docs.cohere.com/v2/reference/rerank) 的模型。

```ts
const model = cohere.reranking('rerank-v3.5');
```

你可以使用 Cohere 重排序模型通过 `rerank` 函数对文档进行重排序：
