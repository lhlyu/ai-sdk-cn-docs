...
你可以从 `@ai-sdk/replicate` 导入默认的提供商实例 `replicate`：
...
<Note>
  不同模型对 `size` 和其他参数的支持情况有所不同。请查看
  模型在 [Replicate](https://replicate.com/explore) 上的文档，了解支持的选项以及可通过
  `providerOptions.replicate` 传递的额外参数。
</Note>
...
Replicate 通过各种模型支持图像编辑。通过 `prompt.images` 传递输入图像，以转换或编辑现有图像。
...
<Note>
  ...
  base64 编码的字符串提供。不同的 Replicate 模型具有不同的参数名称
  和功能——请查看模型在
...
- **maxWaitTimeInSeconds** _number_ - 在同步模式下等待预测完成的最长时间（秒）。默认情况下，Replicate 使用 [同步模式](https://replicate.com/docs/topics/predictions/create-a-prediction#timeout-duration)，超时时间为 60 秒。设置为正数以使用自定义持续时间（例如，`120` 表示 2 分钟）。未指定时，使用默认的 60 秒等待时间。
- **guidance_scale** _number_ - 无分类器引导的引导比例。较高的值会使输出更紧密地匹配提示词。
- **num_inference_steps** _number_ - 去噪步数。步数越多 = 质量越高但速度越慢。
- **negative_prompt** _string_ - 负面提示词，用于指导生成中应避免的内容。
- **output_format** _'png' | 'jpg' | 'webp'_ - 输出图像格式。
- **output_quality** _number (1-100)_ - 输出图像质量。仅适用于 jpg 和 webp。
- **strength** _number (0-1)_ - 图像到图像转换的强度。较低的值会保留更多原始图像。
